...
class HttpServerVerticle : CoroutineVerticle() {
  override suspend fun start() {
    ...
    router.get("/").handler(this) { ctx -> listUploadedFilesHandler(storageService, ctx) }
    router.getWithRegex(".+\\.html").handler(templateHandler)
    router.get("/files/:file").handler(this) { ctx -> serveFileHandler(storageService, ctx) }
    router.post("/")
      .handler(BodyHandler.create())
      .handler(this) { ctx -> fileUploadHandler(storageService, ctx) }
